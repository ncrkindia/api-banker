 ...
 *
 * <p>
 * This class acts as the centralized persistent Cookie store for the ApiBanker HTTP
 * Client.
 * It intercepts incoming HTTP `Set-Cookie` headers from responses, parses their
 * attributes
 * (Domain, Path, Max-Age, Secure), and automatically injects them into
 * subsequent outgoing
 * requests that match the domain and path requirements. It persists cookies
 * locally to `~/.apibanker/cookies.json`.
 * </p>
 *
...
public class CookieJar {
    ...
    private CookieJar() {
        ...
        File bootstrapDir = new File(userHome, ".apibanker");
    ...
    private void loadCookies() {
        if (cookiesFile.exists()) {
            try (Reader reader = new FileReader(cookiesFile, StandardCharsets.UTF_8)) {
                List<CookieModel> loaded = gson.fromJson(reader, new TypeToken<List<CookieModel>>() {
                }.getType());
                if (loaded != null) {
     ...
     * HTTP `Cookie` header string containing all valid, non-expired cookies
     * matching
     * the domain and path.
     * 
     * @param urlString The target URL being requested.
     * @return A formatted `Cookie` header string (e.g., "session=123; user=abc"),
