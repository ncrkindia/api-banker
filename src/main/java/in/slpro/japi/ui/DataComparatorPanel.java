...
public class DataComparatorPanel extends JPanel {
    ...
    private void compare() {
        ...
        boolean isDark = "dark".equals(in.slpro.japi.storage.StorageManager.getInstance().getSettings().getTheme());
        ...
        StringBuilder html = new StringBuilder("<html><body style='font-family:JetBrains Mono,monospace;font-size:" + currentFontSize + "px;background:" + textBg + ";color:" + textColor + ";'>");
        int leftIdx = 0;
        for (AbstractDelta<String> delta : patch.getDeltas()) {
            while (leftIdx < delta.getSource().getPosition()) {
                html.append("<div style='background:").append(textBg).append(";padding:1px 4px;'>").append(escapeHtml(leftLines.get(leftIdx))).append("</div>");
                leftIdx++;
            }
            for (String line : delta.getSource().getLines()) {
                html.append("<div style='background:").append(delBg).append(";padding:1px 4px;color:").append(delText).append(";'>- ").append(escapeHtml(line)).append("</div>");
                leftIdx++;
            }
            for (String line : delta.getTarget().getLines()) {
                html.append("<div style='background:").append(addBg).append(";padding:1px 4px;color:").append(addText).append(";'>+ ").append(escapeHtml(line)).append("</div>");
            }
        }
