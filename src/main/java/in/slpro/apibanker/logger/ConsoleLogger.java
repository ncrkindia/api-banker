    ...
    public void logRequest(String method, String url, int statusCode, long durationMs,
        ...
        if (requestHeaders != null) {
            requestHeaders.forEach(
                    (k, v) -> sb.append("  ").append(k).append(": ").append(String.join(", ", v)).append("\n"));
        }
        if (requestBody != null && !requestBody.isEmpty()) {
            sb.append("Body: ").append(requestBody.length() > 500 ? requestBody.substring(0, 500) + "..." : requestBody)
                    .append("\n");
        }
        ...
        if (responseBody != null && !responseBody.isEmpty()) {
            sb.append("Body: ")
                    .append(responseBody.length() > 1000 ? responseBody.substring(0, 1000) + "..." : responseBody);
        }

        LogEntry.Level level = (statusCode >= 400 || statusCode == 0) ? LogEntry.Level.ERROR : LogEntry.Level.REQUEST;
        long responseSize = responseBody != null ? responseBody.getBytes(java.nio.charset.StandardCharsets.UTF_8).length
                : 0;
        ...
        if (logsDir != null && enableLogging) {
            try {
                ...
                File logFile = new File(logsDir, "apibanker_" + dateStr + ".log");
                try (PrintWriter pw = new PrintWriter(
                        new FileWriter(logFile, java.nio.charset.StandardCharsets.UTF_8, true))) {
                    pw.println("["
