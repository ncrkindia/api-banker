...
public class CollectionPanel extends JPanel {
    ...
    private void buildUI() {
        ...
        linkBtn.addActionListener(e -> {
            String text = JOptionPane.showInputDialog(this, "Enter Link Text:", "Insert Link",
                    JOptionPane.PLAIN_MESSAGE);
            if (text != null && !text.trim().isEmpty()) {
                String url = JOptionPane.showInputDialog(this, "Enter Link URL:", "Insert Link",
                        JOptionPane.PLAIN_MESSAGE);
                if (url != null) {
        ...
        oauth2GetTokenBtn.addActionListener(e -> {
            ...
            in.slpro.apibanker.http.OAuth2Manager.getNewAccessToken(collectionModel, this);
        });
        ...
        sslVerifyCombo.setToolTipText(
                "Select SSL verification behavior for this collection/folder. 'Inherit' resolves to parent collection/folder setting recursively, or global setting.");
    ...
    private void updateEditorFont() {
        if (readmeArea == null || mdFontCombo == null || mdFontSizeCombo == null)
            return;
    ...
    private void updateHtmlPreview() {
        if (htmlPane == null || readmeArea == null)
            return;
        String markdownText = readmeArea.getText();
        if (markdownText == null || markdownText.trim().isEmpty()) {
            htmlPane.setText(
                    "<html><body><p style='color: #888888; font-style: italic; font-family: sans-serif;'>No description provided. Click 'Edit' to add one.</p></body></html>");
            return;
        }
        ...
        try {
            ...
            styleSheet.addRule(
                    "body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif; font-size: "
                            + baseFontSize + "px; color: " + fgHex + "; background-color: " + bgHex
                            + "; margin: 15px; line-height: 1.6; }");
            styleSheet
                    .addRule("h1 { font-size: 1.5em; font-weight: bold; color: " + fgHex + "; border-bottom: 1px solid "
                            + borderHex + "; padding-bottom: 6px; margin-top: 24px; margin-bottom: 12px; }");
            styleSheet
                    .addRule("h2 { font-size: 1.3em; font-weight: bold; color: " + fgHex + "; border-bottom: 1px solid "
                            + borderHex + "; padding-bottom: 4px; margin-top: 20px; margin-bottom: 10px; }");
            styleSheet.addRule("h3 { font-size: 1.15em; font-weight: bold; color: " + fgHex
                    + "; margin-top: 16px; margin-bottom: 8px; }");
            styleSheet.addRule("p { margin-top: 0px; margin-bottom: 12px; }");
            styleSheet.addRule("a { color: " + accentHex + "; text-decoration: none; }");
            styleSheet.addRule(
                    "code { font-family: 'JetBrains Mono', 'Courier New', monospace; font-size: 0.9em; background-color: "
                            + codeBgHex + "; padding: 2px 4px; border-radius: 3px; }");
            styleSheet.addRule(
                    "pre { font-family: 'JetBrains Mono', 'Courier New', monospace; font-size: 0.9em; background-color: "
                            + codeBgHex + "; border: 1px solid " + borderHex
                            + "; padding: 12px; border-radius: 6px; display: block; margin-bottom: 12px; }");
            styleSheet.addRule("blockquote { border-left: 4px solid " + accentHex
                    + "; margin: 0 0 12px 0; padding-left: 12px; color: " + mutedHex + "; font-style: italic; }");
