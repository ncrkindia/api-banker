# JAPI Release Plan — v1.4.0-beta

This document outlines the release plan for **JAPI (v1.4.0-beta)**, the offline-first API client, detailing the beta entry criteria, verification methods, distribution strategy, and milestones for the final stable release.

---

## 1. Release Goals

JAPI's transition to the `1.4.0-beta` pre-release phase aims to achieve the following:
...
This release introduces comprehensive workflow enhancements and project stabilization leading up to v1.4.0-beta.

**Features & Enhancements:**
- **Scripting Expansions**: Added full support for `japi.collectionVariables` inside pre-request and test scripts, enabling dynamic scoping of variables with native snippet UI insertion.
...
- **Version Bump**: Incrementally updated project schemas, pom.xml, Readme, and release plans to track milestone version 1.4.0-beta.
...
* **Target executable**: `japi-1.4.0-beta.jar` (Shaded fat JAR containing all dependencies).
* **Package formats**: `.zip` and `.tar.gz` archive containing launch scripts.
* **Launch scripts**:
  - `japi.bat`: Script to run the application on Windows (`javaw -jar japi-1.4.0-beta.jar`).
...
gantt
    ...
    section Pre-Release
    Beta Release (v1.4.0-beta)     :active, milestone, 2026-08-04, 1d
