...
public class ScriptExecutor {
    ...
    public ScriptResult executePreRequestScript(String script, RequestModel request, EnvironmentModel environment) {
        ...
        try {
            ...
            result.setError(err);
            if (!silentMode && in.slpro.japi.logger.ConsoleLogger.getInstance().isEnableLogging()) {
                in.slpro.japi.logger.ConsoleLogger.getInstance().logMessage(LogEntry.Level.ERROR, err, "Pre-request");
            }
        } catch (Exception e) {
    ...
    public ScriptResult executeTestScript(String script, RequestModel request, ResponseModel response, EnvironmentModel environment) {
        ...
        try {
            ...
            result.setError(err);
            if (!silentMode && in.slpro.japi.logger.ConsoleLogger.getInstance().isEnableLogging()) {
                in.slpro.japi.logger.ConsoleLogger.getInstance().logMessage(LogEntry.Level.ERROR, err, "Test");
            }
        } catch (Exception e) {
    ...
    private void injectPmObject(Context cx, Scriptable scope, RequestModel request,
        ...
        envObj.put("get", envObj, new BaseFunction() {
            ...
            public Object call(Context cx, Scriptable scope, Scriptable thisObj, Object[] args) {
                if (args.length < 1 || environment == null) return Undefined.instance;
        ...
        envObj.put("set", envObj, new BaseFunction() {
            @Override
            public Object call(Context cx, Scriptable scope, Scriptable thisObj, Object[] args) {
                if (args.length < 2 || environment == null) return Undefined.instance;
                String key = Context.toString(args[0]);
        ...
        envObj.put("unset", envObj, new BaseFunction() {
            @Override
            public Object call(Context cx, Scriptable scope, Scriptable thisObj, Object[] args) {
                if (args.length < 1 || environment == null || environment.getVariables() == null) return Undefined.instance;
                String key = Context.toString(args[0]);
        ...
        if (isTestContext && response != null) {
            ...
            resHeaders.put("get", resHeaders, new BaseFunction() {
                @Override
                public Object call(Context cx, Scriptable scope, Scriptable thisObj, Object[] args) {
                    if (args.length < 1 || response.getHeaders() == null) return Undefined.instance;
    ...
    private Scriptable buildExpectChain(Context cx, Scriptable scope, Object actual) {
        ...
        BaseFunction equalFn = new BaseFunction() {
            @Override
            public Object call(Context cx, Scriptable scope, Scriptable thisObj, Object[] args) {
                if (args.length < 1) return chain;
                Object expected = args[0];
                boolean equals = deepEquals(actual, expected);
                if (negated[0]) equals = !equals;
                if (!equals) {
                    throw Context.reportRuntimeError("Expected " + stringify(actual) + (negated[0] ? " to not equal " : " to equal ") + stringify(expected));
                }
                return chain;
            }
        };
        ...
        BaseFunction includeFn = new BaseFunction() {
            ...
            public Object call(Context cx, Scriptable scope, Scriptable thisObj, Object[] args) {
                ...
                if (negated[0]) includes = !includes;
                if (!includes) {
                    throw Context.reportRuntimeError("Expected \"" + actualStr + "\"" + (negated[0] ? " to not include " : " to include ") + "\"" + expected + "\"");
                }
                return chain;
            }
        };
        ...
        be.put("a", be, new BaseFunction() {
            ...
            public Object call(Context cx, Scriptable scope, Scriptable thisObj, Object[] args) {
                ...
                if (negated[0]) match = !match;
                if (!match) {
                    throw Context.reportRuntimeError("Expected " + stringify(actual) + (negated[0] ? " to not be a " : " to be a ") + expectedType + " but got " + actualType);
                }
                return chain;
            }
        });
        ...
        be.put("below", be, new BaseFunction() {
            ...
            public Object call(Context cx, Scriptable scope, Scriptable thisObj, Object[] args) {
                ...
                if (negated[0]) ok = !ok;
                if (!ok) {
                    throw Context.reportRuntimeError("Expected " + actualNum + (negated[0] ? " to not be below " : " to be below ") + expected);
                }
                return chain;
            }
        });
        be.put("above", be, new BaseFunction() {
            ...
            public Object call(Context cx, Scriptable scope, Scriptable thisObj, Object[] args) {
                ...
                if (negated[0]) ok = !ok;
                if (!ok) {
                    throw Context.reportRuntimeError("Expected " + actualNum + (negated[0] ? " to not be above " : " to be above ") + expected);
                }
                return chain;
            }
        });
        ...
        have.put("property", have, new BaseFunction() {
            ...
            public Object call(Context cx, Scriptable scope, Scriptable thisObj, Object[] args) {
                ...
                if (negated[0]) hasProp = !hasProp;
                if (!hasProp) {
                    throw Context.reportRuntimeError("Expected object" + (negated[0] ? " to not have property " : " to have property ") + "'" + propName + "'");
                }
                return chain;
            }
        });
        ...
        have.put("status", have, new BaseFunction() {
            ...
            public Object call(Context cx, Scriptable scope, Scriptable thisObj, Object[] args) {
                ...
                if (negated[0]) ok = !ok;
                if (!ok) {
                    throw Context.reportRuntimeError("Expected status code " + actualCode + (negated[0] ? " to not be " : " to be ") + expectedCode);
                }
                return chain;
            }
        });
        ...
        have.put("lengthOf", have, new BaseFunction() {
            ...
            public Object call(Context cx, Scriptable scope, Scriptable thisObj, Object[] args) {
                ...
                int actualLen = 0;
                if (actual instanceof String s) actualLen = s.length();
                else if (actual instanceof NativeArray arr) actualLen = (int) arr.getLength();
                else if (actual instanceof Scriptable s) {
                    Object lenProp = s.get("length", s);
                    if (lenProp instanceof Number n) actualLen = n.intValue();
                }
                boolean ok = actualLen == expectedLen;
                if (negated[0]) ok = !ok;
                if (!ok) {
                    throw Context.reportRuntimeError("Expected length " + actualLen + (negated[0] ? " to not equal " : " to equal ") + expectedLen);
                }
                return chain;
            }
        });

        // Wire up the chain: chain.to -> to, to.be -> be, to.have -> have, to.not -> not
        to.put("be", to, be);
        to.put("have", to, have);
        to.put("not", to, not);
    ...
    private BaseFunction createConsoleLogFn(ScriptResult result, LogEntry.Level level, String source) {
        return new BaseFunction() {
            ...
            public Object call(Context cx, Scriptable scope, Scriptable thisObj, Object[] args) {
                ...
                if (!silentMode && in.slpro.japi.logger.ConsoleLogger.getInstance().isEnableLogging()) {
                    in.slpro.japi.logger.ConsoleLogger.getInstance().logMessage(level, "[Console] " + msg, source);
                }
                return Undefined.instance;
            }
        };
    ...
    private String stringify(Object o) {
        if (o == null || o instanceof Undefined) return "undefined";
    ...
    private String getJsType(Object o) {
        ...
        if (o instanceof Undefined) return "undefined";
        if (o instanceof Boolean) return "boolean";
        if (o instanceof Number) return "number";
        if (o instanceof String || o instanceof CharSequence) return "string";
        if (o instanceof NativeArray) return "array";
        if (o instanceof Function) return "function";
        if (o instanceof Scriptable) return "object";
        return "object";
    }
