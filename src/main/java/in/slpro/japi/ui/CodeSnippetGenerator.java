...
public class CodeSnippetGenerator {
    ...
    public static String generate(String language, RequestModel request, EnvironmentModel env, boolean resolveVars) {
        ...
        if (!("GET".equals(method) || "DELETE".equals(method) || "HEAD".equals(method))) {
            ...
            if ("raw".equalsIgnoreCase(bodyType)) {
                ...
                if ("JSON".equalsIgnoreCase(rawType)) contentType = "application/json";
                else if ("XML".equalsIgnoreCase(rawType)) contentType = "application/xml";
                else if ("HTML".equalsIgnoreCase(rawType)) contentType = "text/html";
    ...
    private static String generateCurl(String method, String url, Map<String, String> headers, String body) {
        ...
        for (Map.Entry<String, String> entry : headers.entrySet()) {
            sb.append(" \\\n  -H \"").append(entry.getKey()).append(": ").append(escapeCurlHeader(entry.getValue())).append("\"");
        }
    ...
    private static String generateFetch(String method, String url, Map<String, String> headers, String body) {
        ...
        for (Map.Entry<String, String> entry : headers.entrySet()) {
            sb.append("myHeaders.append(\"").append(entry.getKey()).append("\", \"").append(escapeJs(entry.getValue())).append("\");\n");
        }
        sb.append("\n");
        if (body != null && !body.isEmpty()) {
    ...
    private static String generatePython(String method, String url, Map<String, String> headers, String body) {
        ...
        if (!headers.isEmpty()) {
            sb.append("headers = {\n");
            for (Map.Entry<String, String> entry : headers.entrySet()) {
                sb.append("  '").append(entry.getKey()).append("': '").append(escapePython(entry.getValue())).append("',\n");
            }
            sb.append("}\n\n");
        } else {
            sb.append("headers = {}\n\n");
        ...
        if (body != null && !body.isEmpty()) {
            if (headers.get("Content-Type") != null && headers.get("Content-Type").contains("application/json")) {
                ...
                sb.append("response = requests.request(\"").append(method).append("\", url, headers=headers, json=payload)\n\n");
            } else {
                sb.append("payload = ").append(formatPythonBody(body, false)).append("\n\n");
                sb.append("response = requests.request(\"").append(method).append("\", url, headers=headers, data=payload)\n\n");
            }
        } else {
    ...
    private static String generateJava(String method, String url, Map<String, String> headers, String body) {
        ...
        sb.append("public class JapiClientExample {\n");
        ...
        for (Map.Entry<String, String> entry : headers.entrySet()) {
            sb.append("                .header(\"").append(entry.getKey()).append("\", \"").append(escapeJava(entry.getValue())).append("\")\n");
        }
        if (body != null && !body.isEmpty()) {
            sb.append("                .method(\"").append(method).append("\", HttpRequest.BodyPublishers.ofString(").append(formatJavaBody(body)).append("))\n");
        } else {
