...
import in.slpro.japi.ui.RequestPanel;
import in.slpro.japi.ui.CollectionPanel;
        ...
        try {
            if ("client_credentials".equals(grantType) || "password".equals(grantType)) {
                ...
                if ("password".equals(grantType)) {
                    body.append("&username=").append(URLEncoder.encode(username == null ? "" : username, StandardCharsets.UTF_8));
                    body.append("&password=").append(URLEncoder.encode(password == null ? "" : password, StandardCharsets.UTF_8));
                }

                if ("body".equals(clientAuth)) {
                    body.append("&client_id=").append(URLEncoder.encode(clientId == null ? "" : clientId, StandardCharsets.UTF_8));
                    body.append("&client_secret=").append(URLEncoder.encode(clientSecret == null ? "" : clientSecret, StandardCharsets.UTF_8));
                }
                ...
                client.sendAsync(reqBuilder.build(), HttpResponse.BodyHandlers.ofString())
                        ...
                        .exceptionally(ex -> {
                            SwingUtilities.invokeLater(() -> {
                                JOptionPane.showMessageDialog(panel, "Failed to get token: " + ex.getMessage(), "OAuth2 Error", JOptionPane.ERROR_MESSAGE);
                            });
                            return null;
                        });
                ...
                url.append("&client_id=").append(URLEncoder.encode(clientId == null ? "" : clientId, StandardCharsets.UTF_8));
                ...
                try {
                    ...
                    if (cbUri.getPort() > 0) port = cbUri.getPort();
                } catch (Exception e) {}
                ...
                JOptionPane.showMessageDialog(panel, "Grant type '" + grantType + "' is not fully automated. Please generate manually.", "Info", JOptionPane.INFORMATION_MESSAGE);
            }
        } catch (Exception e) {
            JOptionPane.showMessageDialog(panel, "Error preparing OAuth2 request: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    
    private static void startAuthServer(int port, String callbackUrl, String accessTokenUrl, String clientId, String clientSecret, String clientAuth, Object model, JPanel panel) {
        try {
            ...
            server.createContext("/", exchange -> {
                ...
                String responseMsg = "<html><body><h2>JAPI Authorization</h2>";
                if (code != null) {
                    responseMsg += "<p>Authorization code received. You can close this tab and return to JAPI.</p></body></html>";
                    ...
                    exchangeCodeForToken(code, callbackUrl, accessTokenUrl, clientId, clientSecret, clientAuth, model, panel);
                } else {
            ...
            SwingUtilities.invokeLater(() -> {
                JOptionPane.showMessageDialog(panel, "Failed to start local callback server on port " + port + ": " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            });
        }
    }
    
    private static void exchangeCodeForToken(String code, String callbackUrl, String accessTokenUrl, String clientId, String clientSecret, String clientAuth, Object model, JPanel panel) {
        try {
            StringBuilder body = new StringBuilder();
            ...
            client.sendAsync(reqBuilder.build(), HttpResponse.BodyHandlers.ofString())
                    ...
                    .exceptionally(ex -> {
                        SwingUtilities.invokeLater(() -> {
                            JOptionPane.showMessageDialog(panel, "Failed to exchange code for token: " + ex.getMessage(), "OAuth2 Error", JOptionPane.ERROR_MESSAGE);
                        });
                        return null;
                    });
        } catch (Exception e) {
    ...
    private static void handleTokenResponse(String responseBody, Object model, JPanel panel) {
        ...
        if (token != null) {
            ...
            SwingUtilities.invokeLater(() -> {
                ...
                JOptionPane.showMessageDialog(panel, "Access Token successfully generated and saved!", "Success", JOptionPane.INFORMATION_MESSAGE);
            });
        } else {
            SwingUtilities.invokeLater(() -> {
                JOptionPane.showMessageDialog(panel, "Could not extract access_token from response:\n" + responseBody, "OAuth2 Error", JOptionPane.WARNING_MESSAGE);
            });
        }
    }
}
