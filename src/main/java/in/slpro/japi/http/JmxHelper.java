...
public class JmxHelper {
    ...
    public static List<RequestModel> importJmx(File file) throws Exception {
        ...
        for (int i = 0; i < samplerList.getLength(); i++) {
            ...
            if (node.getNodeType() == Node.ELEMENT_NODE) {
                ...
                if (name == null || name.isBlank()) name = "JMeter Request " + (i + 1);
                ...
                if (protocol == null || protocol.isBlank()) protocol = "http";
    ...
    public static void exportJmx(CollectionModel col, File file) throws Exception {
        ...
        addStringProp(doc, testPlan, "TestPlan.comments", "Exported from JAPI");
        ...
        for (RequestModel req : col.getRequests()) {
            if ("runner".equalsIgnoreCase(req.getType())) continue;
            ...
            try {
                ...
                if (uri.getScheme() != null) protocol = uri.getScheme();
                if (uri.getHost() != null) domain = uri.getHost();
                if (uri.getPort() != -1) port = String.valueOf(uri.getPort());
