...
public class CollectionRunnerLogsPanel extends JPanel {
    ...
    public CollectionRunnerLogsPanel(MainFrame mainFrame) {
        ...
        // Only HTML and CSV are easily generated from logs. PDF/Excel requires charts/POI which need UI context or complex libs.
    ...
    private void exportFile(String fileName) {
        ...
        if (!source.exists()) {
            JOptionPane.showMessageDialog(this, fileName + " does not exist for this run. Perhaps it was not saved.", "Info", JOptionPane.INFORMATION_MESSAGE);
            return;
        }
        ...
        if (chooser.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
            try {
                Files.copy(source.toPath(), chooser.getSelectedFile().toPath(), java.nio.file.StandardCopyOption.REPLACE_EXISTING);
    ...
    private void exportGeneratedMetrics(String format) {
        ...
        if (!sumFile.exists()) {
            JOptionPane.showMessageDialog(this, "No summary.log found to generate metrics.", "Info", JOptionPane.INFORMATION_MESSAGE);
            return;
        }

        JFileChooser chooser = new JFileChooser();
        String safeName = selectedRunDir.getParentFile().getName() + "-" + selectedRunDir.getName() + "-metrics." + format;
        chooser.setSelectedFile(new File(safeName));
        if (chooser.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
            try {
                ...
                if ("csv".equalsIgnoreCase(format)) {
                    ...
                    JOptionPane.showMessageDialog(this, format.toUpperCase() + " generation from logs is not supported yet.", "Info", JOptionPane.INFORMATION_MESSAGE);
                    return;
                }
                MainFrame.showToast(this, "Metrics (" + format.toUpperCase() + ") generated and exported successfully.");
            } catch (Exception e) {
                JOptionPane.showMessageDialog(this, "Generation failed: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            }
        }
    }
    ...
    private void generateCSV(List<LogItem> items, File dest) throws Exception {
        try (java.io.PrintWriter pw = new java.io.PrintWriter(new java.io.FileWriter(dest, java.nio.charset.StandardCharsets.UTF_8))) {
            pw.println("\"JAPI Generated Metrics Report\"");
            pw.println("\"Generated At:\",\"" + java.time.LocalDateTime.now().toString() + "\"");
            pw.println();
            pw.println("Time,Method,API Name,Status,Latency (ms)");
            for (LogItem item : items) {
                pw.println(String.format("\"%s\",\"%s\",\"%s\",%d,%d", item.time, item.method, item.name, item.status, item.latency));
            }
        }
    }
    ...
    private void generateHTML(List<LogItem> items, File dest) throws Exception {
        try (java.io.PrintWriter pw = new java.io.PrintWriter(new java.io.FileWriter(dest, java.nio.charset.StandardCharsets.UTF_8))) {
            pw.println("<html><head><title>JAPI Metrics</title>");
            pw.println("<style>body{font-family:sans-serif;margin:20px;} table{width:100%;border-collapse:collapse;} th,td{border:1px solid #ddd;padding:8px;text-align:left;} th{background-color:#f4f4f4;} .pass{color:green;} .fail{color:red;}</style>");
            pw.println("</head><body>");
            pw.println("<h2>JAPI Generated Metrics Report</h2>");
            pw.println("<table><tr><th>Time</th><th>Method</th><th>API Name</th><th>Status</th><th>Latency (ms)</th></tr>");
            for (LogItem item : items) {
