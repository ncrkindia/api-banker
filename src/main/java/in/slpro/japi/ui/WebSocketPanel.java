...
import in.slpro.japi.model.VariableHelper;
...
public class WebSocketPanel extends JPanel {
    ...
    private class WebSocketListener implements WebSocket.Listener {
        ...
        public CompletionStage<?> onClose(WebSocket webSocket, int statusCode, String reason) {
            SwingUtilities.invokeLater(() -> {
                addLog(MessageLogItem.Type.DISCONNECT, "Closed by remote peer. Status: " + statusCode + ", Reason: " + reason);
                disconnect();
            });
            return null;
        }
    ...
    private static class MessageLogItem {
        enum Type { CONNECT, DISCONNECT, SENT, RECEIVED, ERROR }
    ...
    private static class MessageLogRenderer extends DefaultListCellRenderer {
        ...
        public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
            ...
            if (value instanceof MessageLogItem item) {
                ...
                setToolTipText("<html><pre>" + item.text.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;") + "</pre></html>");
            }
            return this;
        }
    }
