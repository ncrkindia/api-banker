...
public class ConsoleDialog extends JDialog implements ConsoleLogger.LogListener {
    ...
    public ConsoleDialog(JFrame owner) {
        ...
        searchField.getDocument().addDocumentListener(new DocumentListener() {
            @Override public void insertUpdate(DocumentEvent e) { filterLogs(); }
            @Override public void removeUpdate(DocumentEvent e) { filterLogs(); }
            @Override public void changedUpdate(DocumentEvent e) { filterLogs(); }
        });
    ...
    private void filterLogs() {
        ...
        for (LogEntry entry : ConsoleLogger.getInstance().getEntries()) {
            ...
            if ("Logs/Info".equals(filterLevel)) {
                if (entry.getUrl() != null || entry.getLevel() == LogEntry.Level.ERROR) continue;
            } else if ("Errors".equals(filterLevel)) {
                if (entry.getLevel() != LogEntry.Level.ERROR) continue;
            ...
            if (!search.isEmpty()) {
                ...
                if (!matchMsg && !matchUrl && matchMethod && !matchSource) continue;
            }
    ...
    private JPanel createCollapsibleRequestPanel(LogEntry entry) {
        ...
        JLabel statusLabel = new JLabel(entry.getStatusCode() == 0 ? "Network Error" : String.valueOf(entry.getStatusCode()));
    ...
    private JPanel createFormattedRequestPanel(LogEntry entry) {
        ...
        String resLine = "Status: " + (entry.getStatusCode() == 0 ? "Network Error" : entry.getStatusCode() + " (" + entry.getDurationMs() + " ms)");
    ...
    private Color getSourceColor(String source) {
        if (source == null) return new Color(127, 140, 141);
    ...
    public void onLogEntry(LogEntry entry) {
        SwingUtilities.invokeLater(() -> {
            ...
            if ("Logs/Info".equals(filterLevel)) {
                if (entry.getUrl() != null || entry.getLevel() == LogEntry.Level.ERROR) return;
            } else if ("Errors".equals(filterLevel)) {
                if (entry.getLevel() != LogEntry.Level.ERROR) return;
            } else if ("Network Requests".equals(filterLevel)) {
                if (entry.getUrl() == null) return;
            }
            ...
            if (!search.isEmpty()) {
                ...
                boolean matchMethod = entry.getMethod() != null && entry.getMethod().toLowerCase().contains(search);
                boolean matchSource = entry.getSource() != null && entry.getSource().toLowerCase().contains(search);
                if (!matchMsg && !matchUrl && !matchMethod && !matchSource) return;
            }
